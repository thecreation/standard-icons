...
module.exports = class IconModule extends Generator {
  ...
  initializing() {
    // this.log("It looks like you are adding a new Icon module!")
  }
  ...
  paths() {
    this.basePath = this.destinationPath(stripPrefix(this.options.module))
  }
  ...
  writing() {
    ...
    data.lib = stripPrefix(data.module)
    ...
    let src = path.join(this.basePath, "scss/module.scss")
    let dest = src.replace("module.scss", `${data.lib}.scss`)
    this.fs.move(src, dest)

    src = path.join(this.basePath, "module.config.json")
    dest = src.replace("module.config.json", `${data.lib}.config.json`)
    this.fs.move(src, dest)

    src = path.join(this.basePath, "module.hbs")
    dest = src.replace("module.hbs", `${data.lib}.hbs`)
    this.fs.move(src, dest)
  }
  ...
  _addAsDependencyTo(pkg, dest) {
    ...
    const destPath = require.resolve(
      path.join("../../../modules/", stripPrefix(dest), "package.json")
    )
