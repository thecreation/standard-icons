...
let options = {
  ...
  class: 'ril',
  prefix: 'ril-',
  className: 'RemixiconLine',
  title: 'Remixicon Line',
  author: 'Remix Design',
...
options.svgs = getSvgs(paths.svgs, '**/*-line.svg');
...
module.exports = function(callback) {
  ...
  categories.forEach(category => {
    icons[category] = [];
    let svgs = getSvgs(path.join(paths.svgs, category), '*-line.svg').map(svg => svg.replace(/\.svg$/, ''));
    ...
    svgs.sort((a, b) => {
      ...
      icons[category].push({
        name: svg.replace(/-line/, ''),
        content: iconsMap[svg],
        title: svg.replace(/-line/, '')
      });
    })
  })

  clean(paths.dest)
  options.svgs = copySvgs(paths.svgsDest, paths.svgs, options.svgs, '-line')
